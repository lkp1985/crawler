...
public class CompanyMain2 {
	private static final Log LOGGER = LogFactory.getLog(CompanyMain2.class);
...
//		executor.execute(new RunThread());
	//	PhoneUtil.init();
		QYSDUtil.init();
		while(true){
			executor.execute(new RunThread2());
			//getJYScope4();
		}
		//writerCompanyInfo();
	}
	...
	//	System.out.println("companyMap-="+companyMap);
		for(String id :companyMap.keySet()){
			...
			String scope = QYSDUtil.getJYScope2(regcode);
	...
	public static void runList(String address, String mainurl) {
		try {
			...
			headParams.put("Accept-Encoding", "gzip, deflate");// "gzip,
																// deflate,
																// sdch");
			headParams.put("Cookie", CompanyMain2.cookie);
...
class RunThread2 implements Runnable {
	...
	public void run() {
		try{
			System.out.println(Thread.class.getName()+" start");
			Map<String,String> companyMap = MysqlUtil.getCompanyQueryCondition2();
			Map<String,String> newmap = new HashMap<String,String>();
		//	System.out.println("companyMap-="+companyMap);
			for(String id :companyMap.keySet()){
			...
			CompanyMain2.successnum += newmap.size();
			System.out.println("successnum="+CompanyMain2.successnum);
			if(newmap.size()>0){
				MysqlUtil.insertScope(newmap);
			}
