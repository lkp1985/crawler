		...
		while(true){
			try{
				...
				for(int i=0;i<time ; i++){
					...
					System.out.println(Thread.currentThread() +",MainThread.proxyRequestTimes.get(key)="+MainThread.proxyRequestTimes.get(key));
					if(MainThread.proxyRequestTimes.get(key)>=50){
						
						while((System.currentTimeMillis() -
								MainThread.proxyLastRequestTime.get(key))<60000 ||
								MainThread.proxyRequestTimes.get(key)>=50){
							Thread.sleep(10000);
						}
					}
					System.out.println("begin request");
					MainThread.proxyLastRequestTime.put(key, System.currentTimeMillis());
					MainThread.proxyRequestTimes.put(key, 1);
